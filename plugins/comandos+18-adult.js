...
if (!db.data.chats[m.chat].modohorny && m.isGroup) throw `${lenguajeGB['smsAvisoAG']()}𝑳𝒐𝒔 𝒄𝒐𝒎𝒂𝒏𝒅𝒐𝒔 +18 𝒆𝒔𝒕𝒂𝒏 𝒅𝒆𝒔𝒂𝒄𝒕𝒊𝒗𝒂𝒅𝒐𝒔 𝒆𝒏 𝒆𝒔𝒕𝒆 𝒄𝒉𝒂𝒕\n\n𝙐𝙨𝙖: #𝙤𝙣 𝙢𝙤𝙙𝙤𝙝𝙤𝙧𝙣𝙮 𝙋𝙖𝙧𝙖 𝙖𝙘𝙩𝙞𝙫𝙖𝙧𝙡𝙤 𝙚𝙣 𝙚𝙡 𝙘𝙝𝙖𝙩`   
if (command == 'nsfwloli') {
let haha = await conn.getFile(`https://server-api-rey.herokuapp.com/api/wallpaper/nsfwloli?apikey=apirey`) 
conn.sendButton(m.chat, `_${command}_`.trim(), wm, haha.data, [['🥵 𝙎𝙄𝙂𝙐𝙄𝙀𝙉𝙏𝙀', `/${command}`]], m)}
if (command == 'nsfwfoot') {
...
let json = res.data
let url = json.url
conn.sendButton(m.chat, `_${command}_`.trim(), wm, url, [['🥵 𝙎𝙄𝙂𝙐𝙄𝙀𝙉𝙏𝙀', `/${command}`]], m)}
if (command == 'booty') {
